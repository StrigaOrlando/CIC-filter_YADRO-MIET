module cic_decimator_top #(
    ...
    ((((OUTPUT_MODE == 0) ? OUT_WIDTH :
       (NORMALIZE ? IN_WIDTH :
        cic_parameters_pkg::calc_full_width(IN_WIDTH, N, R, M))) + 7) / 8) * 8
) (
    input  logic                           i_clk        ,
    input  logic                           i_rst_n      ,
    ...
    output logic [TDATA_OUT_W-1:0]         o_dout_tdata ,
    output logic                           o_dout_vld   ,
    input  logic                           i_dout_tready
);

    localparam GROWTH_W   = cic_parameters_pkg::calc_bit_growth(N, R, M)          ;
    localparam FULL_WIDTH = cic_parameters_pkg::calc_full_width(IN_WIDTH, N, R, M);
    ...
    generate
        if (OUTPUT_MODE == 0) begin : gen_round
            cic_round #(
                .IN_WIDTH   (EFFECTIVE_WIDTH) ,
                .OUT_WIDTH  (ACTUAL_OUT_WIDTH),
                .ROUND_MODE (ROUND_MODE)
            ) u_round (
                .clk          (i_clk)                         ,
                .rst_n        (i_rst_n)                       ,
                .i_din        (norm_dout[EFFECTIVE_WIDTH-1:0]),
    ...
    generate
        if (ACTUAL_OUT_WIDTH >= TDATA_OUT_W) begin : gen_wide_out
            ...
            assign o_dout_tdata = {{(TDATA_OUT_W - ACTUAL_OUT_WIDTH){round_dout[ACTUAL_OUT_WIDTH-1]}}, round_dout};
        end
    endgenerate
